...
import { addressEquals, routingDistance, formatAddress, } from '../address/index.js';
...
export class Router extends EventEmitter {
    ...
    addRoute(route) {
        ...
        const existingIndex = routeList.findIndex((r) => addressEquals(r.nextHop, route.nextHop));
        if (existingIndex >= 0) {
    ...
    removeRoute(destination, nextHop) {
        ...
        if (nextHop) {
            ...
            const index = routeList.findIndex((r) => addressEquals(r.nextHop, nextHop));
            if (index >= 0) {
     ...
     */
    findMultipleRoutes(destination, count = 3) {
        const routes = [];
        // Direct routes
        const destKey = this.getAddressKey(destination);
        const directRoutes = this.routes.get(destKey);
     ...
     */
    processRouteReply(packet, receivedFrom) {
     ...
     */
    async requestRoute(destination, timeoutMs = 5000) {
     ...
     */
    handlePeerDisconnect(peerAddress) {
        // Remove all routes via this peer
        for (const [destKey, routeList] of this.routes) {
            const filteredRoutes = routeList.filter((r) => !addressEquals(r.nextHop, peerAddress));
     ...
     */
    learnRoute(packet, receivedFrom) {
        const { source } = packet.header;
