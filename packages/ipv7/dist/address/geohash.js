 ...
 */
export function encode(latitude, longitude, precision = 4) {
    // Validate inputs
...
export function neighbors(geohash) {
    ...
    return directions.map(({ lat, lon }) => encode(latitude + lat, longitude + lon, precision));
}
/**
 * Calculate approximate distance between two geohashes in kilometers
 */
export function distance(geohash1, geohash2) {
 ...
 */
export function commonPrefixLength(geohash1, geohash2) {
    let length = 0;
 ...
 */
export function contains(geohash1, geohash2) {
